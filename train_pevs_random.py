...
from pev_battery_charge.envs.PEVBatteryChargeRandom import PEVBatteryChargeRandom
...
def make_env(args, rank, seed=0):
    ...
    """
    def _init():
        env = PEVBatteryChargeRandom(args)
...
def main():
    ...
    args2 = Namespace(**vars(args).copy()) # For security
    args2.train_random = False
    
    env_eval = PEVBatteryChargeRandom(args2)

    if args.n_rollout_threads == 1:
        # if there is only one process, there is no need to use multiprocessing
        env = DummyVecEnv([lambda: PEVBatteryChargeRandom(args)])
    else:
